...
CControlUI::~CControlUI()
{
	...
	m_pManager ->KillTimer(this, DUI_TIMERID_ANIMATION);
}
...
void CControlUI::SetDislplay(bool dislplay)
{
	m_display = dislplay;
}
bool CControlUI::IsDisplay()
{
	return m_display;
}

void CControlUI::SetInterval(int interval)
{
	m_interval = interval;
}

int CControlUI::GetInterval()
{
	return m_interval;
}

void CControlUI::SetIncreaseValue(int value)
{
	m_increaseValue = value;
}

int CControlUI::GetIncreaseValue()
{
	return m_increaseValue;
}

void CControlUI::SetAnimation(bool animation)
{
	m_animation = animation;
}

bool CControlUI::GetAnimation()
{
	return m_animation;
}

void CControlUI::SetAutoPlay(bool autoplay)
{
	m_autoPlay = autoplay;
}

bool CControlUI::GetAutoPlay()
{
	return m_autoPlay;
}
...
void CControlUI::OnTimer(UINT_PTR idEvent)
{
	if (m_display)
	{
		m_transparency += m_increaseValue; 
	...
	std::wstring imagepath = m_sBkImage;
	if (imagepath.find(L"file") != std::wstring::npos)
	{
		imagepath = imagepath.substr(imagepath.find(L"file='") + wcslen(L"file='"));
		imagepath = imagepath.substr(0, imagepath.find(L'\''));
	}
	wsprintf(str, L"file='%s' fade='%d'", imagepath.c_str(), m_transparency);
#else
	memset(str, 0, MAX_PATH);
	std::string imagepath = m_sBkImage;
	if (imagepath.find("file") != std::string::npos)
	{
		imagepath = imagepath.substr(imagepath.find("file='") + strlen("file='"));
		imagepath = imagepath.substr(0, imagepath.find('\''));
	}
	sprintf(str, "file='%s' fade='%d'", imagepath.c_str(), m_transparency);
#endif
...
void CControlUI::DoEvent(TEventUI& event)
{
    ...
    {
		if ((UINT_PTR)event.wParam == DUI_TIMERID_ANIMATION)
		{
...
bool CControlUI::StartAnimation()
{
	if (!GetAnimation() || GetIncreaseValue() == 0 || GetInterval() == 0)
	{
		return false;
	}
	if (IsDisplay())
	...
	m_pManager->SetTimer(this, DUI_TIMERID_ANIMATION, m_interval);
	return true;
}

bool CControlUI::StartAnimation(int interval, int increaseValue, bool display)
{
	if (!m_animation || increaseValue == 0 || interval == 0)
	{
		return false;
	}
	m_interval = interval;
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	//渐变动画相关
	else if( _tcscmp(pstrName, _T("animation")) == 0 ) SetAnimation(_tcscmp(pstrValue, _T("true")) == 0);
	else if( _tcscmp(pstrName, _T("display")) == 0 ) SetDislplay(_tcscmp(pstrValue, _T("true")) == 0);
	else if( _tcscmp(pstrName, _T("increasevalue")) == 0 ) SetIncreaseValue(_ttoi(pstrValue));
