...
namespace DuiLib
{
	...
	public:
		...
		//收起所有一级节点
		virtual void CloseAllChild();
		//展开关闭时已经展开的一级节点
		virtual void OpenAllChild();
		//移动节点到指定位置
		bool MovePlace(CTreeNodeUI* from, CTreeNodeUI* to);
	private:
		UINT m_uItemMinWidth;
		bool m_bVisibleFolderBtn;
		bool m_bVisibleCheckBtn;
		std::map<CTreeNodeUI*, bool> m_nodeStateList;		//存储节点状态 展开or关闭
		bool m_brecordNodeState;							//标记只记录一次节点信息
	};
}
