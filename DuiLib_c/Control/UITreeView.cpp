...
namespace DuiLib
{
	...
	// 函数说明: 
	//************************************
	CTreeNodeUI::CTreeNodeUI( CTreeNodeUI* _ParentNode /*= NULL*/ ) : m_bIsMoved(false), m_bCanMove(false)
	{
	...
	void CTreeNodeUI::DoEvent( TEventUI& event )
	{
		...
		if( event.Type == UIEVENT_BUTTONUP ) 
		{
			...
			if( (m_uButtonState & UISTATE_CAPTURED) != 0  && IsEnabled())   
			{
				...
				{
					CTreeViewUI* parent = static_cast<CTreeViewUI*>(m_pOwner);
					auto control = GetFirstCTreeNodeUIFromPoint(event.ptMouse);
					if (control != NULL)
					{
						CTreeNodeUI* target = static_cast<CTreeNodeUI*>(GetFirstCTreeNodeUIFromPoint(event.ptMouse));
						parent->MovePlace(static_cast<CTreeNodeUI*>(this), target);
					}
					m_uButtonState &= ~UISTATE_CAPTURED;  
					if( m_pManager )   
					{    
	...
	void CTreeNodeUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
	{
		...
		else if (_tcscmp(pstrName, _T("canmove")) == 0 )
			SetCanMove(_tcscmp(pstrValue, _T("true")) == 0 );
	...
	// 函数说明: 绘制阴影部分
	//************************************
	void CTreeNodeUI::DoPostPaint(HDC hDC, const RECT& rcPaint)
	{  
		if( (m_uButtonState & UISTATE_CAPTURED) != 0 && m_bIsMoved) {  
	...
	//************************************
	// 函数名称: GetFirstCTreeNodeUIFromPoint
	// 返回类型: CTreeNodeUI*
	// 参数信息: POINT pt
	// 函数说明: 返回指针位置顶点节点
	//************************************
	...
	// 函数说明: 设置移动属性
	//************************************
	void CTreeNodeUI::SetCanMove(bool canmove)
	{
		m_bCanMove = canmove;
	}
	//************************************
	...
	// 函数说明: 
	//************************************
	CTreeViewUI::CTreeViewUI( void ) : m_bVisibleFolderBtn(true),m_bVisibleCheckBtn(false),m_uItemMinWidth(0), m_brecordNodeState(false)
	{
	...
	// 函数说明: 收起所有一级节点
	//************************************
	void CTreeViewUI::CloseAllChild()
	{
		if (m_brecordNodeState)
		{
			return;
		}
		m_nodeStateList.clear();
		...
		{
			CTreeNodeUI* targetNode = static_cast<CTreeNodeUI*>(GetItemAt(i));
			if (targetNode->IsHasChild())
			{
				if(targetNode->GetVisibleTag())
				{
					m_nodeStateList[static_cast<CTreeNodeUI*>(GetItemAt(i))] = true;
				}
				SetItemExpand(false, static_cast<CTreeNodeUI*>(GetItemAt(i)));
				static_cast<CTreeNodeUI*>(GetItemAt(i))->SetVisibleTag(false);
			}
		}
		m_brecordNodeState = true;
	}
	//************************************
	// 函数名称: OpenAllChild
	// 返回类型: void
	// 函数说明: 展开节点到上一状态
	//************************************
	void CTreeViewUI::OpenAllChild()
	{
		for (auto iter = m_nodeStateList.begin(); iter != m_nodeStateList.end(); ++iter)
		{
			SetItemExpand(true, iter->first);
			iter->first->SetVisibleTag(true);
		}
		m_brecordNodeState = false;
	}
	//************************************
	// 函数名称: MovePlace
	// 返回类型: bool
	// 函数说明: 移动节点到指定位置
	//************************************
	bool CTreeViewUI::MovePlace(CTreeNodeUI* from, CTreeNodeUI* to)
	{
		...
		//目标节点不是一级节点
		if(to->GetParentNode()!=NULL)
		{
			return false;
		}
		

		auto parent = from->GetParentNode();
		//同一节点不移动
		if (parent->GetIndex() == to->GetIndex())
		{
			return true;
		}
		//删除
		...
		//整理索引
		for(int i = from->GetIndex() == 0 ? 0 : from->GetIndex() - 1; i < m_pList->GetCount(); ++i) {
			CControlUI* p = m_pList->GetItemAt(i);
			auto pListItem = static_cast<IListItemUI*>(p->GetInterface(_T("CTreeNodeUI")));
			if( pListItem != NULL ) {
		...
		from->SetParentNode(to);
		//整理索引
		for(int i = to->GetIndex() == 0 ? 0 : to->GetIndex() - 1; i < m_pList->GetCount(); ++i) {
			CControlUI* p = m_pList->GetItemAt(i);
			auto pListItem = static_cast<IListItemUI*>(p->GetInterface(_T("CTreeNodeUI")));
			if( pListItem != NULL ) {
				pListItem->SetIndex(i);
