...
#define CURRENTLY_ON_MAIN_THREAD() \
    reeiss_cef::MainMessageLoop::Get()->RunsTasksOnCurrentThread()

#define REQUIRE_MAIN_THREAD() DCHECK(CURRENTLY_ON_MAIN_THREAD())

#define MAIN_POST_TASK(task) \
    reeiss_cef::MainMessageLoop::Get()->PostTask(task)

#define MAIN_POST_CLOSURE(closure) \
    reeiss_cef::MainMessageLoop::Get()->PostClosure(closure)
...
struct DeleteOnMainThread {
  ...
  static void Destruct(const T* x) {
    if (CURRENTLY_ON_MAIN_THREAD()) {
      ...
      reeiss_cef::MainMessageLoop::Get()->PostClosure(
